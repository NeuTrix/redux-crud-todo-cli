...
describe('The TodoReducer CRUD suite', () => {
	...
	afterAll(() => {
		initialTodoList.length > 1 ? TodoList.pop() : TodoList;
	});
	...
	describe.only('The ADD_TODO function', () => { 
		...
		beforeAll(() => {
			...
			modifiedTodoList = store.getState().todos;
			modLength = modifiedTodoList.length;
			_task= modifiedTodoList[modLength -1];
			_id = _task._id
		});

		it('... the new "modifiedTodoList" is an array', () => {
			expect(modifiedTodoList).to.be.an('array');
		})

		it('...can ADD one more _task to the initialTodoList', () => {
			let initialLength = initialTodoList.length
	...
	describe.only('The REMOVE_TODO function', () => {
		...
		it.only('_task is a todo object with an id', () => {
			expect(task).to.be.have.property("_id")
		})
